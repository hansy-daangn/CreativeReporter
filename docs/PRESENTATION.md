...
- **효율 지도** (산점도): 기본 축 = 가로 **CTR** · 세로 **단가**(eCPA→eCPI→전환당비용)
...
- **목적 정렬**: 매체별 1순위 = **획득 단가**
  ...
  - **Google AEO** → **CPI** (설치당 비용)
  - **Google ACE** → **CPA** (인앱액션당 비용)
  - **ROAS는 채점에서 제외** — 전환가치가 모델링 값이라 신뢰 낮음 (표·지도엔 참고용만)
- **점수 = 효율 75% + 규모 20% + 신뢰 5%**
- **공정성 장치 3가지**:
  1. **광고비 가중** — 돈 많이 쓴 소재가 기준선을 정한다
  2. **표본 축소** — 전환·클릭 적은 소재는 점수를 중앙으로 당겨 과대·과소평가 방지
  3. **코호트 비교** — CTR·완주율은 **같은 포맷(영상 vs 이미지)** 끼리만
...
여기가 이 툴의 심장입니다. 우리의 목적은 "신규·복귀 유저를 효율적으로 데려온 소재를 찾는 것"이죠. 그래서 채점의 1순위를 매체마다 **획득 단가**로 뒀습니다 — 몰로코와 메타는 신규+재활성 유저 한 명당 비용, Google AEO는 설치당 비용 CPI, ACE는 인앱액션당 비용 CPA입니다. 예전엔 ROAS(광고비 대비 매출)를 크게 봤는데, 그 전환가치는 실제 매출이 아니라 모델이 추정한 값이라 신뢰가 낮습니다. 그래서 점수에서는 빼고, 참고용으로 표와 지도에만 남겼습니다.
...
가중치(SCORING) — 매체별 획득 단가 1순위, ROAS 제외:
  ad(몰로코·메타):
    신규+재활성 eCPA .40 / 활성 eCPA .22 / CTR .18 / 완주율 .08(영상만) / eCPI .06 / CPM .06
  google_aeo(신규):  eCPI(CPI) .50 / CTR .22 / 전환율 .16 / CPM .12
  google_ace(리타겟): 전환당비용(CPA) .50 / CTR .22 / 전환율 .16 / CPM .12
  google(유형 미상):  전환당비용 .46 / CTR .22 / 전환율 .18 / CPM .14
