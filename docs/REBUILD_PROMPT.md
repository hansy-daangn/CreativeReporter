...
- **소재 편집(관리자 연필 팝오버)**: 별 오른쪽에 연필(`.mark-btn.edit`, 관리자·개별 소재만). 클릭 → body에 붙는 팝오버(anchor 아래)로 **이름·서비스·제작자** 편집. 저장: 이름→cr_names_merge(NAME_OVR), 서비스→cr_svc_merge(SVC_OVR, ''=자동), 제작자 추가/삭제 diff→cr_admin_mark_set(특정인 mine). 저장 후 renderInlineRefresh(재렌더+열린 상세 재개+필터 개수 갱신). 바깥클릭·Esc 닫기.
...
- **서비스·제작자 복수선택 드롭다운**(툴바): 커스텀 msel(체크박스, 선택 토글 시 팝업 유지 — 옵션 목록 시그니처가 바뀔 때만 재구성). 서비스=svcOf 버킷별 개수, 제작자=내 소재 등록자별 개수(mineCreatorsOf, 마크 있을 때만 노출). applyTFilter에서 tServices/tCreators(Set) 교집합 필터.
