...
-- cr_pending_count(pw): perform _cr_check_pw(pw) 후 신청함 수 반환(사이트 알림용, anon grant).
...
- 관리자 페이지 `admin.html`: 별도 자기완결 파일(공개 URL, 로그인 `</> Dev`로도 진입). 관리자 비번 게이트(런타임 입력, 파일 비저장) → 표(신청함 먼저·상태·**권한 뷰어/에디터 토글**·**비밀번호 열람**·신청일·처리자). 행별 승인/거절(**확인창 없이 즉시**)/대기(`cr_admin_set` p_status)+권한(p_role)+**비번변경**(`cr_admin_setpw`). 상단 **＋계정 추가**(`cr_admin_create`, 승인됨 생성)·**비밀번호 보기** 토글(기본 가림·클릭 복사). 비번은 pgcrypto로 암호화 저장(pw_enc)하고 `cr_admin_list`가 복호화해 반환 — 관리자만 열람. 암호화 전 가입자는 `—`(비번변경으로 재설정). 거절은 상태 보존(재승인 가능). **열려 있는 동안 15초 폴링** → 새 신청 시 토스트+브라우저 알림(unlock 시 Notification 권한 요청). 메인 화면 에디터에게도 `cr_pending_count`로 우하단 알림 칩(→admin.html).
