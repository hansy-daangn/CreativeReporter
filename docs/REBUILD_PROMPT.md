...
- 상세(행 클릭 아코디언, B안 히어로): 구조 = grid-template-areas 3단계 반응형, DOM 순서=중요도(미디어→같은 그룹 비교→주차별 추이→점수·진단): 와이드(>1200) "media cmp trend"/"media cmp sd" 3열(미디어 열 ar-v 232/ar-s 280/ar-h 400px) · 미디엄(≤1200) "media cmp"/"sd cmp"/"trend trend" 2열(미디어 260/가로 340px, 추이 전폭) · 모바일(≤860) 중요도 순 1열 스택+프레임 폭 캡(320px, 가로 420px)+중앙 정렬. 하단 `지표별 계정 내 위치`(.sb 2열 columns:2·break-inside:avoid, 모바일 1열). 비교표 이름 셀은 줄바꿈 허용(white-space:normal·break-word). 리사이즈 시 열린 상세의 픽셀 폭을 .tbl-scroll clientWidth로 재설정(전역 resize 리스너). 이름·뱃지·칩·내비 버튼 등 헤더 요소 없음(행이 이미 보여줌). 점수는 38px 대형 + 회색 `/ 100 · 계정 상위 N%`(N=rank/eligCount, 50% 초과면 '하위'로 표기). 진단 한 줄·주차별 추이 미니차트는 점수 아래. 미디어: 고정 프레임 .isle-frame(정방 기본 280px, .isle의 ar-v→232px 폭·396px 높이, ar-h→400px 폭·250px 높이 그리드 전환), img는 max-width/height contain, video는 width/height 100%+object-fit:contain+다크 배경, 유튜브는 iframe 100%. 비율 판정: 몰로코 _m_res·유튜브(가로)는 렌더 즉시, 이미지류는 onload에서 naturalWidth/Height로 h/w≥1.2 세로·w/h≥1.2 가로·그 외 정방(isleAr/applyIsleAr). 이미지 실패 시 즉시(0ms) wsrv.nl 프록시 전환→1.2s 원본 캐시우회→1.2s 프록시 캐시우회→프레임 안 실패 표시+수동 재시도, 프록시 성공은 localStorage `crImgProxy` 기기 학습. 그룹 상세: isle-hero.grp = [점수·진단·추이|멤버 비교|보고서 추가 정보(캠페인·타겟 CPA·참여율·first_open·home·평균 시청·영상 25/50/75/100% 퍼널)], 미디어 없음. 코호트 순위 막대·지표 토글은 없음(지표 위치 2열이 대체). 키보드 ←/→ = 표의 현재 필터·정렬 순서로 이전/다음(페이지 경계 자동), Esc 닫기, 입력창 포커스 중 무시. 상세 열림 시 history.replaceState로 `#ad=<encodeURIComponent(name)>&ch=<media>` 기록·닫으면 제거, 로드 후 해시 있으면 해당 매체 전환+자동 오픈, 없는 소재면 토스트.
