...
```
원점수 = 100 × (0.85·E + 0.15·R)   ← 신뢰 축 없음(표본 보정은 지표별 축소로 일원화)
E: 지표별 g′의 가중평균. g = 해당 지표의 광고비 가중 백분위(모수: 값 보유 전체 소재, 가중치 max(비용,1))
   g′ = 0.5 + (g − 0.5)·n/(n+n₀). n = 지표 표본(전환류=전환수, 클릭류=클릭수, 노출류=노출수, D1=코호트 base), n₀ = 30~50/100/2000/150
   구글 단가류(_g_cpHome/_g_costPerConv/_g_cpFirstOpen/_g_hpm)는 지출 기반 축소 병행: w = min(표본 w, 비용/(비용+1,000,000)) — SPEC_EVIDENCE den2/n02
   방향: 비용류(-1)는 1−백분위. CTR·CPM·CPC·완주율·상호작용률·홈 도달율류는 같은 포맷(영상/이미지) 코호트 ≥8개일 때 코호트 분포로.
   포맷 조건부(entFo): AEO CTR=영상 소재만 · 지면 조건부(entCt): 몰로코 CTR=_m_ctype "IMAGE"(외부 지면 이미지)만 채점
     (타 포맷·지면은 해당 지표 건너뜀·재정규화. entCt 지표의 코호트 축은 fo 대신 _m_ctype — specCohortOf(s,e)).
   지표 신뢰성 게이트(aggregate 단계): ① 구글 소재의 CTR>0.15(CTR_PLAUSIBLE_MAX)면 노출 과소집계 이상으로 판정,
     IMP_RATE_KEYS(CTR·CPM·_g_hpm·상호작용률·조회율·전환율·_g_foRate)를 null 처리하고 클릭≥50이면 _impBad=1(⚠ 칩).
     ② 몰로코 영상(_m_ctype에 VIDEO 포함, fo≠이미지)은 클릭에 재생 혼입 → CTR·CPC null + _clkMix=1.
     그룹 집계(buildAdGroupAgg)·AB 변형 합산도 오염 멤버가 하나라도 있으면 같은 키를 null(게이트 상속).
R: '데려온 유저'의 광고비 가중 백분위 — 몰로코·메타=신규+재활성 유저 수(AF), AEO=_g_firstOpen, ACE=_g_home(홈 도달). 가용도 <50%면 노출 폴백.
가중치 — moloco: 신규+재활성 eCPA .70, CTR .12(외부 지면 이미지 IMAGE만), 완주율 .12(영상만), D1 잔존율 .06
       meta: 신규+재활성 eCPA .60, D1 잔존율 .22, 신규 설치 단가(_q_cpNewInstall) .18 (CTR 제거 — 실측 역방향)
       google_aeo: 신규 단가(_g_cpFirstOpen) .64, 홈 단가(_g_cpHome) .12, 신규 전환율(_g_foRate) .12, CTR .12(영상만)
       google_ace(기본 홈 기준): 홈 단가(_g_cpHome) .55, 홈 도달율(_g_hpm=홈÷노출×1000) .20, CPM .25 — 전환율 금지(뷰스루 혼입), 전환 CPA는 토글 변형
       google(유형미상 폴백): 전환당 비용 .72, CTR .14, 홈 도달율 .14
       adset: 활성 eCPA .35, eCPI .25, CTR .20, CPM .15, CPC .05 · cg: CTR .50, CPC .30, CPM .20
주차별 점수: 총점과 동일한 자(전체 기간 분포·같은 스펙·같은 재배치)로 그 주 지표를 채점(weeklyScoresOf) — 주별 재상대평가 금지.
...
표시: 모든 점수는 소수점 1자리(fmtScore). 상세 '점수 구성'은 재배치 점수와 지표별 백분위 막대만 — 원점수·가중치·계산식 노출 금지(산정 방식은 푸터로). 전체 목록 점수 칸은 십의 자리별 10단계 색(sd0~sd9). 무점수(score null) 소재는 기본 숨김('비활성 포함' 체크로 표시, 채점 없는 소규모 데이터셋은 예외).
tier: 적격자 점수 5분위(g2/g1/m/w1/w2) + dom '독보적'(1위가 2위와 압도적 격차 && 비용 ≥ 중앙값).
학습중 배지: 비용>0 && (집행 주 <2 || (채점됐으면 1순위 지표 evidenceW<0.5, 아니면 전환 <30)).
그룹 집계 행도 동일 스펙으로 채점.
```
...
- 관리자 페이지 `admin.html`: 별도 자기완결 파일(공개 URL, 로그인 `</> Dev`로도 진입). 관리자 비번 게이트(런타임 입력, 파일 비저장) → **한 줄 행 표**(컬럼 '이름'·상태·권한·비밀번호·액션 / th,td nowrap). 권한은 **뷰어/에디터 2토글**(승인 기본 viewer) — **관리자(role=admin)는 콘솔에서 부여·강등 불가**, 해당 계정은 주황 `관리자` 태그로만 표시(afRole에도 admin 없음). 행별 승인/거절(**확인창 없이 즉시**, `cr_admin_set` p_status)+**이름 ✎수정**(p_name, 변경 시 마크·업로드 서명까지 서버에서 연쇄 갱신)+**비번변경**(`cr_admin_setpw`). **대기 버튼 없음** — 승인·거절을 누르지 않은 신청함 상태가 곧 대기. **신청일·처리 열은 상단 '상세 보기' 체크 시에만 표시**(.det). 반응형: ≤860 비밀번호 숨김 → ≤660 비번변경·✎ 숨김 → ≤520 권한 숨김(이름+승인/거절이 최후까지 유지). 상단 **＋계정 추가**(`cr_admin_create`, 승인됨 생성). 비번 열은 평소 ●●●, **호버 시 표시**(pw_enc 복호화값, 클릭 복사) — 암호화 전 가입자는 `—`. 처리 열은 날짜만(처리자 미표기). 거절은 상태 보존(재승인 가능). **열려 있는 동안 15초 폴링** → 새 신청 시 토스트+브라우저 알림(unlock 시 Notification 권한 요청). 가입 알림은 **관리자 페이지 전용**(메인 화면엔 없음).
...
- **자료 추출(사이드바 `내 자료 추출하기`, CSV 버튼은 아이콘화)**: 중앙 플로팅 모달 — 제작자(모두+로스터, 단일 칩) · 매체(가로 세그먼트, 순서 모두·몰로코·구글·메타 = _chRank 고정, 복수 · 모두=전체) · 기간(가로 세그먼트 1달/3달/전체/직접, 직접=`<input type=date>` 시작~종료 절대구간, 기본=데이터 min~max) · 비교 데이터 3단 세그먼트(none/summary/full, 기본 summary, 제작자=모두면 none 강제·비활성) · 최소 점수 세그먼트(''/40/50/60/70). 절대구간은 scopeRange(tmp,ch,start,end)로 재집계·재채점. summary 모드 B = expBench 기준선(개수·중앙값·등급분포·대상 델타) + expStatTable(지표별 중앙값·25/75백분위 3행) + 상위 3개 expTable. full 모드 B = 광고비 상위 40개 표. 최소 점수는 A 표에서만 필터(요약 통계는 전체 기준 + "제외 N개" 명시). 순위 셀은 #r/N(N=그 매체·기간 rank 보유 수). 헤더에 비교 모드·최소점수·"통계 행은 개별 소재 아님" 명시. 추출: adDs 복제→tmp.media=채널·periodN=선택기간→scope(tmp)로 매체별 재채점, curDS=tmp로 잠시 바꿔 displayNameOf/svcOf/rosterCreatorsOf가 채널 정확하게, 매체별 A(대상)·B(비교군) 섹션 Markdown 생성 → downloadText(Blob .md). 각 소재는 자기 행에만(혼용 금지), 매체 다르면 점수 비교 금지 명시. 끝나면 periodN·curDS 복원 + show(active).
