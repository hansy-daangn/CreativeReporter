...
- 상세(행 클릭 아코디언): 구조 = `.isle-top`(2분할 grid: [미디어 열(ar-v 232/ar-s 280/ar-h 400px) it-side(프레임+원본링크+점수·진단) | it-cmp(같은 그룹/시리즈 비교)]) + `.isle-bot`(구분선 border-top #E1DFD7, 3분할 시각: [ib-trend 1fr | .ipos 2fr(내부 .sb 2열 columns:2)]). 점수는 30px+회색 `/ 100 · 계정 상위 N%`. 비교 목록: table-layout:fixed(수치 열 90/82/62px)+한 줄 ellipsis, prefixTrimmer(그룹 표시 이름들의 공통 접두를 토큰 경계(≥10자)에서 '…'로 접기, tr title=풀네임). 추이 없으면 .isle-bot.noTrend 1열. 모바일(≤860): 전부 1열 스택(미디어→비교→추이→지표), 프레임 max-width 320(가로 420)px 중앙, 지표 1열. 이미지 실패·비율 판정·키보드·딥링크 동작은 기존과 동일.
