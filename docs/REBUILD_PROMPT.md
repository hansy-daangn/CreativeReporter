...
표시: 모든 점수는 소수점 1자리(fmtScore). 상세 '점수 구성' 헤더에 원점수와 재배치 점수를 함께 표기.
...
- 분석: 요약 타일 3개, 효율 지도(축 선택 산점도+범위 슬라이더, 상위 광고비 라벨), 주차별 추세(선택 기간 전체, 라벨 ≤13 자동 솎기, 광고비 면적+효율선 이중축), 유형별 비교(상하 10% 트림 평균).
...
- 상세(행 클릭 아코디언): 미디어(영상 재생/이미지. 이미지 실패 시 즉시(0ms) wsrv.nl 프록시 전환→1.2s 원본+캐시우회→1.2s 프록시+캐시우회→실패 표시+수동 재시도 버튼. 프록시 성공 시 localStorage `crImgProxy` 학습→그 기기는 재방문에도 처음부터 프록시 렌더, 모든 img에 `data-osrc` 원본 보존. 호버 툴팁 유튜브 썸네일은 default.jpg 즉시 표시 후 hqdefault 로드 완료 시 교체(data-hi)) + 주차별 추이(고정폭 얇은 막대=광고비 + 매체별 자동 선택 지표선: 전환 데이터 보유(구글) ROAS·전환당 비용 2선 / 앱 채널 eCPI·활성 eCPA·CTR 3선, 없는 지표 자동 제외·전무 시 CPC, 선별 자체 최대값 정규화, 범례에 최신값, 월 경계 라벨 'YY-M') / 대표 지표 토글(표 대표 지표와 동일 세트·순서)+코호트 순위 막대(전체·서비스·포맷·사이즈·그룹·시리즈) / 같은 그룹·시리즈 표(풀네임, 클릭 이동, 호버 미리보기) / 점수 구성 패널(지표별 백분위·가중치·축소 표시 + 진단 한 줄: 표본보정 아닌 지표 중 백분위 ≥70 상위 2개=강점·≤30 하위 1개=보완, 광고비 집행 주 수=운영 N주차(첫 집행 주 툴팁), 최근 4주 CTR 연속 하락+마지막 주 집행 유지 시 '피로 징후 · 소재 리프레시 검토'). 그룹 상세: 코호트 막대 없이 [추이|멤버 비교|보고서 추가 정보(캠페인·타겟 CPA·참여율·first_open·home·평균 시청·영상 25/50/75/100% 퍼널)].
- 상세 내비·딥링크: 상세 우상단 `n / N · ‹ 이전 · 다음 › · 링크 복사`(표의 현재 필터·정렬 순서로 이동, 페이지 경계 자동 전환, ← →/Esc 키, 입력창 포커스 중 무시). 상세 열림 시 `history.replaceState`로 `#ad=<encodeURIComponent(name)>&ch=<media>` 해시 기록·닫으면 제거, 로드 완료 후 해시가 있으면 해당 매체로 전환해 그 소재 상세 자동 오픈(없으면 토스트), 링크 복사는 clipboard API+textarea 폴백.
