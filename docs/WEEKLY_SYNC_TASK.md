...
> 배경·확정된 사실·절대 규칙 요약은 [`CONTEXT.md`](./CONTEXT.md)에 있다. 처음이라면 그것부터 읽어라.
...
## 🔧 먼저 할 일 — 예약 작업 1개로 교체 (사람이 데스크톱 앱에서 직접)

**기존 `월 09:30` 슬롯은 구조적으로 불가능하다.** 그 시각엔 필요한 원천 두 개가 모두 비어 있다(§A). 아래대로 교체한다.

1. Claude 데스크톱 앱 → **설정 → 예약 작업**
2. 기존 CR 주간 동기화 작업 **전부 삭제** (월 09:30 · 12:30 · 16:30 · 화 09:30)
3. 아래 프롬프트로 새 작업 **1개**만 생성 — **월 11:00 (KST)**
...
### 왜 1개인가 — 재시도는 슬롯이 아니라 알림으로 한다

이전 개정판은 같은 작업을 4개 슬롯(월 11·14·17시 + 화 10시)에 걸어 재시도를 흉내 냈다. 데스크톱 예약 작업이 "2시간 뒤 다시"를 스스로 기다리지 못하기 때문인데, **대가가 크다**: 성공한 주에도 세션이 4번 뜬다. 2·3·4번째는 0) 멱등 체크에 걸려 곧바로 끝나므로 1번 성공했을 때의 비용 4배까지는 아니지만, 아무 일도 하지 않는 세션 3개가 매주 실행되는 것은 그대로다.

**대신 사람에게 알린다.** 11:00 실행이 보류·실패로 끝나면 Slack DM이 재실행 프롬프트를 그대로 담아 온다 — 눌러서 붙여넣으면 그 자리에서 재시도된다. 자동 재시도가 사라지는 대신, 재시도가 필요한 주에만 비용이 든다.

| | 이전 | 지금 |
|---|---|---|
| 정상인 주 (대부분) | 세션 4회 | **세션 1회** |
| 실패한 주 | 자동 재시도 3회 | Slack DM → 사람이 1회 재실행 |
| 아무도 안 봤을 때 | 화 10:00까지 자동 재시도 | 월 20:00 클라우드 감시가 푸시 알림(§9) |

11:00은 두 업스트림(콘솔 10:01 · `ad_report` 10:15)이 모두 들어온 뒤라 여유가 50분 이상이다(§A). 실제로 재시도가 필요한 주는 드물다.

> 여러 번 돌려도 안전하다는 성질(0) 멱등 체크)은 그대로다. 손으로 몇 번을 재실행해도 중복 기록되지 않는다.
...
11:00 │ ◀ 실행 (마진 45분) ── 유일한 예약 슬롯
```

실패하면 자동 재시도 대신 Slack DM이 온다(§6). 그때만 사람이 프롬프트를 다시 눌러 실행한다.
...
- **멱등 체크**: Supabase(project `newoydegfbnnqujgiips`)에서
  ...
  — 넷 다 이미 있으면 종료. DM(`[CR] 이미 최신이에요 ✅ (W주)`)은 **그날 첫 실행에서만** 보낸다. 사람이 손으로 여러 번 재실행할 때 같은 알림이 반복되면 소음이다(당일 두 번째 이후 실행은 조용히 종료).
...
- **차단 조건 보류(3-b)**: `[CR] {채널} 보류 ⚠️ {M/D}주 — {P번호} {사유}`

**실패·보류·부분성공 DM에는 반드시 재실행 프롬프트를 그대로 붙인다.** 자동 재시도 슬롯이 없어졌으므로, 이 DM이 유일한 재시도 경로다. 받는 사람이 복사해서 바로 붙여넣을 수 있어야 한다:

```
다시 실행하려면 아래를 데스크톱 앱에 붙여넣어 주세요 (몇 번을 돌려도 안전해요)
https://raw.githubusercontent.com/hansy-daangn/CreativeReporter/main/docs/WEEKLY_SYNC_TASK.md 를 읽고 그대로 수행해줘
```
