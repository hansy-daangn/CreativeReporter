...
**월 11:00 (KST) 한 개.** 구 `월 09:30·12:30·16:30 + 화 09:30` 4개는 폐기했다.

재시도는 슬롯이 아니라 **알림**으로 한다. 4개 슬롯을 걸면 정상인 주(대부분)에도 아무 일 없이 종료되는 세션이 3개 더 뜬다 — 멱등 체크에 걸려 곧 끝나긴 하지만 매주 반복되는 낭비다. 지금은 11:00 실행이 보류·실패로 끝날 때 Slack DM이 **재실행 프롬프트를 그대로 담아** 오고, 사람이 그걸 붙여넣어 재시도한다. 아무도 못 봤을 경우의 안전망은 월 20:00 클라우드 감시(푸시 알림)가 이미 맡고 있다.
...
**감시 장치(클라우드, 설정 완료)**: 매주 월요일 **20:00 KST**에 CCR Routine이 Supabase를 검사한다. 검사는 뷰어 게이트 RPC `cr_sync_status(pw, p_week)`를 curl로 호출한다(Routine 세션은 MCP 커넥터 미보유).

**종전엔 행 수만 셌다.** 그래서 "행은 다 들어왔는데 노출이 0" 같은 값 오류를 통과시켰다 — 2026-07-27주 사고가 정확히 그 형태였다. 지금 RPC는 `{ok, issues[]}`를 돌려주며 **P0~P6**을 검사한다:

| 코드 | 무엇을 잡나 |
|---|---|
| **P0** | 그 주 데이터가 아예 없음 (미적재) |
| P1 | 몰로코 `SUM(노출 수) == SUM(_afImp)` → **폴백 발화** |
| P2 | 몰로코 `SUM(노출 수) == 0` |
| P3 | 어떤 행이든 `클릭 수 > 노출 수` |
| P4 | 콘솔 유래 키(`_v25`~`_v100`·`_m_installs`)가 그 주 전 행에서 전부 빔 |
| P5 | 직전 주 대비 노출 합계 90% 이상 급감 |
| P6 | 필수 키 보유율 99% 미만 |

알림은 두 갈래로 갈린다:

- **P0만** → `[CR] 이번 주 최신화가 아직 안 됐어요 ⏰` (아직 안 돌았다)
- **P1~P6 중 하나라도** → `[CR] 잘못 들어왔어요 ❌` + 걸린 코드·채널 (돌긴 돌았는데 값이 틀렸다)
- 아무것도 없으면 침묵

> **런북 3-b와 역할이 다르다.** 3-b는 **기록 전** 차단(걸리면 아예 안 넣는다), 이 감시는 **기록 후** 독립 검증(이미 들어간 것을 다시 본다). 같은 P 코드를 쓰지만 발동 시점과 대응이 다르므로 둘 중 하나로 합치지 마라 — 3-b를 우회해 들어온 경로(수동 삽입·백필)는 이 감시만 잡는다.
...
| 2 | **Superset 차트 캐시** — 앞선 실행이 폴백 상태 결과를 캐시에 채우면 나중 실행도 그 값을 받는다 | 조회 직전 `force_refresh_chart_cache` 호출 |
