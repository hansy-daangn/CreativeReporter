...
- **표 제목행(소재·점수·광고비·데려온 유저…)을 헤더의 2번째 줄로 고정** — 스크롤해도 툴바 바로 아래에 계속 보이고, 클릭 정렬·`+` 세부지표 펼치기는 그대로. *(기존엔 스크롤 시 제목행이 툴바 뒤로 사라지던 문제를 바로잡음.)*
- 정렬 상태 강조, 키보드 조작(Enter/Space)·`aria-sort` 등 접근성 보강, 좁은 화면 가로 스크롤 안전장치.
